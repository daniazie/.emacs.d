...
#+begin_src emacs-lisp
  ...
  (defun my/gh-link-description (link description)
    (or description
  	  ...
  	  (progn (let ((repo (nth 0 (my/gh-parse-link link)))
  		       (issue (nth 1 (my/gh-parse-link link))))
  		   (format "%s: %s" (string-remove-prefix "gh:" link) (my/gh-get-issue-title repo issue))))
  	link)))

  (defun my/gh-get-issue-title (repo issue)
    (let ((cmd (format "gh issue view %s --repo %s --json title"
  		     (shell-quote-argument issue)
  ...
  (defun my/gh-store-link (link &optional description)
    (org-link-store-props
     ...
     :description (my/gh-description link description)))
...
#+begin_src emacs-lisp
  (with-eval-after-load 'magit
    (require 'forge))
#+end_src
** Create PR and issues
*** org-mode to gfm
#+begin_src emacs-lisp
  (defun my/org-to-gfm ()
    (interactive)
    (goto-char (point))
    (let ((buffer (org-org-export-as-org nil 'subtree))
  	(pyscript (file-truename "~/.emacs.d/configs/convert-org-to-gfm.py")))
      (save-window-excursion
        (get-buffer-create buffer)
        (let* ((body (string-trim (buffer-string)))
  	     (cmd (format "python %s --org %s"
  			  (shell-quote-argument pyscript)
  			  (shell-quote-argument body))))
  	(shell-command-to-string cmd))
        (kill-buffer))))

  (keymap-global-set "C-c g p" #'my/org-to-gfm)
#+end_src
*** heading to title
#+begin_src emacs-lisp
  (defun my/gh-title ()
    ...
    (save-window-excursion
  	...
  	(string-trim (string-trim title "*"))))))
#+end_src
*** Write post
#+begin_src emacs-lisp
  (defun my/gh-args-list (gh-type)
    (let ((gh-args '("assignee" "template" "editor" "project" "recover" "web" "label" "milestone"))
  	(gh-pr-args '("base" "draft" "dry-run" "head"))
  	(gh-issue-args '("parent" "type")))
      (if (string= gh-type "pr")
  	(append gh-args gh-pr-args)
        (append gh-args gh-issue-args))))

  (defun my/gh-additional-args (gh-args)
    (interactive)
    (let ((gh-args-create nil))
      (while gh-args
        (let* ((gh-key (pop gh-args))
  	     (gh-val (read-string (format "%s: " gh-key))))
  	(unless (string-empty-p gh-val)
  	  (setq gh-args-create
  		(plist-put gh-args-create
  			   (intern (format ":%s" gh-key)) gh-val))
  	  gh-args-create)))))

  (defun my/gh-write ()
    (interactive)
    (let* ((create-new (completing-read "Create new: " '("pr" "issue") nil t))
  	 (gh-args-keys (my/gh-args-list create-new))
  	 (create-title (my/gh-title))
  	 (create-body (my/org-to-gfm))
  	 (gh-args (my/gh-additional-args gh-args-keys))
  	 (cmd (format "gh %s create --title %s --body %s "
  		      (shell-quote-argument create-new)
  		      (shell-quote-argument create-title)
  		      (shell-quote-argument create-body))))
      (when gh-args
        (while gh-args
  	(let ((key (pop gh-args))
  	      (val (pop gh-args)))
  	  (setq cmd (concat cmd (format "--%s %s "
  					key
  ...
  (defun my/gh-create ()
    ...
    (let* ((repo (my/gh-select-repo))
  	 (cmd-prefix (call-interactively #'my/gh-write))
  	 (cmd (concat cmd-prefix (format "--repo %s" (shell-quote-argument repo)))))
      (shell-command-to-string cmd)))

  (keymap-global-set "C-c g c" #'my/gh-create)
#+end_src
